...
export default function Home(){
  ...
  return <div className="app-shell">
    <nav className="floating-nav" aria-label="Primary navigation">
      ...
      <div className="nav-links">{NAV.map(([id,label])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id)}><span>{label}</span>{view===id&&<i/>}</button>)}</div>
...
function Dashboard(p:{state:AppState,currentDay:number,completion:number,expected:number,streak:{current:number,longest:number},totalMinutes:number,activeDays:number,totalTasks:number,todayPlan:DayPlan,todayActivity:Activity,notice:string,setView:(v:View)=>void,setSelectedDay:(n:number)=>void}){
 ...
 return <div className="page dashboard"><section className="mission-hero"><span className="eyebrow">DATASPRINT 95 · LEARNING CONTROL ROOM</span><h1><span>Build skills daily.</span><strong>Stay on track.</strong></h1><p>Complete your 95-day data science roadmap, protect your streak and see exactly where you stand.</p></section><section className="welcome"><div><div className="eyebrow">MISSION STATUS · DAY {Math.min(currentDay,95)}</div><h2>Good to have you back, {state.name}.</h2><p>{notice}</p></div><div className="countdown"><span>{Math.max(0,95-currentDay+1)}</span><small>DAYS<br/>REMAINING</small></div></section>
...
function Onboarding({state,update}:{state:AppState,update:(f:(s:AppState)=>AppState)=>void}){
