...
export default function Home(){
  ...
  useEffect(()=>{if(session===undefined)return;let active=true;if(!session){queueMicrotask(()=>{if(active){setReady(true);setSyncStatus("device")}});return()=>{active=false}}queueMicrotask(async()=>{let restored=seedState();try{const saved=localStorage.getItem("datasprint95");if(saved)restored=JSON.parse(saved)}catch(error){console.warn("Could not restore saved DataSprint progress",error)}try{const response=await fetch("/api/progress",{headers:{authorization:`Bearer ${session.access_token}`}});if(response.ok){const remote=await response.json();if(remote.state)restored=remote.state;setSyncStatus("saved")}else setSyncStatus("device")}catch{setSyncStatus("device")}if(active){setState(restored);setReady(true)}});return()=>{active=false}},[session]);
  useEffect(()=>{if(!ready||!session)return;localStorage.setItem("datasprint95",JSON.stringify(state));document.documentElement.dataset.theme=state.theme;const timeout=window.setTimeout(async()=>{setSyncStatus(current=>current==="device"?"device":"saving");try{const response=await fetch("/api/progress",{method:"PUT",headers:{"content-type":"application/json",authorization:`Bearer ${session.access_token}`},body:JSON.stringify(state)});setSyncStatus(response.ok?"saved":"device")}catch{setSyncStatus("device")}},700);return()=>window.clearTimeout(timeout)},[state,ready,session]);
