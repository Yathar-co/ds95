...
type AppState = { name: string; startDate: string; dailyTarget: number; theme: "dark" | "light"; onboarded: boolean; xp: number; activities: Record<string, Activity>; personalTasks: Task[]; freezes: string[] };
type View = "dashboard" | "today" | "syllabus" | "analytics";
...
const NAV:[View,string,string][]=[["dashboard","Overview","⌂"],["today","Today","▶"],["syllabus","Syllabus","▤"],["analytics","Analytics","↗"]];
...
export default function Home(){
  ...
  useEffect(()=>{try{const saved=localStorage.getItem("datasprint95"); if(saved)setState(JSON.parse(saved));}catch{} setReady(true)},[]);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <button className="brand" onClick={()=>setView("dashboard")}><span className="brand-mark">D<span>95</span></span><span>DataSprint <b>95</b></span></button>
      <nav>{NAV.map(([id,label,icon])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id)}><i>{icon}</i><span>{label}</span></button>)}</nav>
      <div className="side-card"><div className="level-line"><span>LVL {level}</span><b>{state.xp.toLocaleString()} XP</b></div><div className="mini-progress"><span style={{width:`${state.xp%500/5}%`}}/></div><small>{500-state.xp%500} XP to level {level+1}</small></div>
      <button className="profile"><span className="avatar">{state.name[0]||"A"}</span><span><b>{state.name}</b><small>Data scientist in training</small></span><span>•••</span></button>
    </aside>
    <main>
      <header className="topbar"><div className="mobile-brand">D<span>95</span></div><div className="top-actions"><button className="icon-btn" aria-label="Toggle theme" onClick={()=>update(s=>({...s,theme:s.theme==="dark"?"light":"dark"}))}>{state.theme==="dark"?"☼":"◐"}</button><button className="icon-btn" aria-label="Notifications">♢<em/></button><span className="avatar small">{state.name[0]||"A"}</span></div></header>
      ...
      {view==="today"&&<Today state={state} plan={todayPlan} activity={todayActivity} update={update} notify={notify}/>} 
    ...
    <nav className="bottom-nav">{NAV.map(([id,label,icon])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id)}><i>{icon}</i><span>{label}</span></button>)}</nav>
    {!state.onboarded&&<Onboarding state={state} update={update}/>} 
...
function Dashboard(p:{state:AppState,currentDay:number,completion:number,expected:number,streak:{current:number,longest:number},totalMinutes:number,activeDays:number,totalTasks:number,todayPlan:DayPlan,todayActivity:Activity,notice:string,setView:(v:View)=>void,setSelectedDay:(n:number)=>void}){
 ...
 return <div className="page dashboard"><section className="welcome"><div><div className="eyebrow">THURSDAY • YOUR LEARNING HUB</div><h1>Welcome back, {state.name} <span>✦</span></h1><p>{notice}</p></div><div className="countdown"><span>{Math.max(0,95-currentDay+1)}</span><small>DAYS<br/>REMAINING</small></div></section>
  ...
  <section className="hero-card">
    <div className="hero-copy"><div className="tag">WEEK {todayPlan.week} • IN PROGRESS</div><h2>{todayPlan.course}</h2><p>Day {Math.min(currentDay,95)} of 95 · {todayPlan.topic}</p><button className="primary" onClick={()=>setView("today")}><span>▶</span> Continue Learning</button></div>
    <ProgressRing value={completion}/>
  </section>
  <section className="metrics">
    <Metric icon="⚡" value={`${streak.current} days`} label="Current streak" sub={`Best: ${streak.longest} days`} accent="amber"/>
    <Metric icon="◷" value={`${Math.floor(totalMinutes/60)}h ${totalMinutes%60}m`} label="Total study time" sub={`Goal: ${Math.round(currentDay*state.dailyTarget/60)}h`} accent="purple"/>
    <Metric icon="✓" value={String(todayActivity.completed.length)} label="Tasks today" sub={`of ${todayPlan.tasks.filter(t=>t.required).length} required`} accent="green"/>
    <Metric icon="◆" value={String(activeDays)} label="Active days" sub={`${totalTasks} tasks completed`} accent="blue"/>
  </section>
...
function Today({state,plan,activity,update,notify}:{state:AppState,plan:DayPlan,activity:Activity,update:(f:(s:AppState)=>AppState)=>void,notify:(s:string)=>void}){
 ...
 return <div className="page today-page"><div className="page-head"><div><span className="eyebrow">DAY {plan.day} · {fmtDate(key)}</span><h1>{plan.topic}</h1><p>{plan.course} · Week {plan.week}</p></div><div className="day-progress"><b>{activity.completed.length}/{plan.tasks.length}</b><span>tasks complete</span></div></div>
...
function Syllabus({state,allDone,completion,update,notify}:{state:AppState,allDone:Set<string>,completion:number,update:(f:(s:AppState)=>AppState)=>void,notify:(s:string)=>void}){
 ...
 return <div className="page syllabus-page"><div className="page-head"><div><span className="eyebrow">178 HOURS · 12 COURSES</span><h1>Your syllabus</h1><p>A complete path from first principles to a job-ready portfolio.</p></div><ProgressRing value={completion}/></div><div className="toolbar"><div className="search">⌕<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search courses and topics…"/></div><select><option>All courses</option><option>In progress</option><option>Completed</option></select></div>
...
function DayModal({day,state,onClose}:{day:number,state:AppState,onClose:()=>void}){const plan=CURRICULUM[Math.min(day,95)-1];const key=iso(addDays(state.startDate,day-1));const a=state.activities[key];return <div className="modal-backdrop" onMouseDown={onClose}><div className="day-modal" onMouseDown={e=>e.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><span className="eyebrow">DAY {day} · {fmtDate(key)}</span><h2>{plan?.topic||"Beyond the target date"}</h2><p>{plan?.course||"Recovery period"}</p><div className="modal-stats"><div><b>{a?.completed.length||0}/{plan?.tasks.length||0}</b><span>Tasks</span></div><div><b>{a?.minutes||0}</b><span>Minutes</span></div><div><b>{active(a)?"Yes":"No"}</b><span>Extended streak</span></div></div><h3>Assigned tasks</h3>{plan?.tasks.map(t=><div className="modal-task" key={t.id}><span>{a?.completed.includes(t.id)?"✓":"○"}</span>{t.title}</div>)}<h3>Notes</h3><p className="note-box">{a?.notes||"No notes recorded for this day."}</p><div className="modal-detail"><span>Exercise result <b>{a?.completed.length?"Completed":"Not recorded"}</b></span><span>Project activity <b>{a?.challenge?"Challenge completed":"None"}</b></span></div></div></div>}

function Onboarding({state,update}:{state:AppState,update:(f:(s:AppState)=>AppState)=>void}){const [step,setStep]=useState(0);const [form,setForm]=useState({name:"",startDate:todayIso(),dailyTarget:120,theme:state.theme,days:["Mon","Tue","Wed","Thu","Fri","Sat"]});const finish=()=>update(s=>({...s,...form,onboarded:true,activities:{},xp:0}));return <div className="onboarding"><div className="onboard-card"><div className="onboard-art"><div className="brand-mark big">D<span>95</span></div><div className="art-grid">{Array.from({length:35},(_,i)=><i key={i} className={i<20?i%5===0?"medium":"lit":""}/>)}</div><span>95 days. One career-changing sprint.</span></div><div className="onboard-form"><div className="step-dots">{[0,1,2].map(i=><i key={i} className={i<=step?"active":""}/>)}</div>{step===0&&<><span className="eyebrow">WELCOME TO DATASPRINT 95</span><h1>Build skills that compound.</h1><p>A focused 95-day path from data science fundamentals to a job-ready machine-learning portfolio.</p><label>What should we call you?<input autoFocus value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Your first name"/></label><button className="primary full" disabled={!form.name.trim()} onClick={()=>setStep(1)}>Build my learning plan →</button><button className="demo-link" onClick={()=>update(s=>({...s,onboarded:true}))}>Explore with demo progress</button></>}{step===1&&<><span className="eyebrow">SET YOUR PACE</span><h1>Make the sprint yours.</h1><label>Preferred start date<input type="date" value={form.startDate} onChange={e=>setForm({...form,startDate:e.target.value})}/></label><label>Daily study target<select value={form.dailyTarget} onChange={e=>setForm({...form,dailyTarget:Number(e.target.value)})}><option value="60">1 hour</option><option value="90">1.5 hours</option><option value="120">2 hours</option><option value="150">2.5 hours</option></select></label><span className="label">Your six study days</span><div className="day-picks">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d=><button key={d} className={form.days.includes(d)?"selected":""} onClick={()=>setForm({...form,days:form.days.includes(d)?form.days.filter(x=>x!==d):form.days.length<6?[...form.days,d]:form.days})}>{d[0]}</button>)}</div><button className="primary full" onClick={()=>setStep(2)}>Continue →</button></>}{step===2&&<><span className="eyebrow">ONE LAST DETAIL</span><h1>Choose your focus mode.</h1><div className="theme-picks"><button className={form.theme==="dark"?"selected":""} onClick={()=>setForm({...form,theme:"dark"})}><span className="dark-preview"/>Dark focus</button><button className={form.theme==="light"?"selected":""} onClick={()=>setForm({...form,theme:"light"})}><span className="light-preview"/>Light clarity</button></div><div className="plan-summary"><b>Your plan is ready</b><span>95 calendar days · 178 hours · 12 courses</span><span>Target finish: {fmtDate(iso(addDays(form.startDate,94)))}</span></div><button className="primary full" onClick={finish}>Start Day 1 →</button></>}</div></div></div>}
