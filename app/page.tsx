...
export default function Home(){
  ...
  return <div className="app-shell">
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      {view==="today"&&<Today plan={todayPlan} activity={todayActivity} completedTopics={state.completedTopics} update={update} notify={notify}/>}
...
function AccountModal({session,syncStatus,onClose}:{session:Session,syncStatus:SyncStatus,onClose:()=>void}){
 ...
 return <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="account-modal" role="dialog" aria-modal="true" aria-labelledby="account-title"><button className="modal-close" onClick={onClose} aria-label="Close account">×</button><span className="eyebrow">YOUR ACCOUNT</span><div className="account-identity"><span>{displayName(session)[0]||"A"}</span><div><h2 id="account-title">{displayName(session)}</h2><p>{session.user.email}</p></div></div><div className="account-status"><i className={syncStatus}/><div><b>{status}</b><small>Protected by Supabase email authentication</small></div></div><div className="account-security"><h3>Security & recovery</h3><p>Email verification and password recovery are managed securely by Supabase. DataSprint never stores your password.</p></div><button className="secondary account-signout" onClick={async()=>{await supabase.auth.signOut();onClose()}}>Sign out</button></section></div>
}
...
function Today({plan,activity,completedTopics,update,notify}:{plan:DayPlan,activity:Activity,completedTopics:string[],update:(f:(s:AppState)=>AppState)=>void,notify:(s:string)=>void}){
...
function Syllabus({state,completion,update,notify}:{state:AppState,completion:number,update:(f:(s:AppState)=>AppState)=>void,notify:(s:string)=>void}){
 ...
 const toggleTopic=(course:string,topic:string)=>{const id=topicProgressId(course,topic);const has=state.completedTopics.includes(id);const key=todayIso();const currentDay=Math.max(1,dayDiff(state.startDate,key)+1);const plan=CURRICULUM[Math.min(currentDay,95)-1];const todayTasks=plan.tasks.filter(task=>task.topicId===id);update(s=>{const completedTopics=has?s.completedTopics.filter(topicId=>topicId!==id):Array.from(new Set([...s.completedTopics,id]));if(!todayTasks.length)return {...s,completedTopics};const current=s.activities[key]||{completed:[],minutes:0,notes:"",difficulty:3,confidence:3};const taskIds=new Set(todayTasks.map(task=>task.id));const completed=has?current.completed.filter(taskId=>!taskIds.has(taskId)):Array.from(new Set([...current.completed,...taskIds]));return {...s,completedTopics,activities:{...s.activities,[key]:{...current,completed}}}});notify(has?"Topic reopened":"Topic complete · Today updated")};
 return <div className="page syllabus-page"><div className="page-head"><div><span className="eyebrow">178 HOURS · 12 COURSES · 95 DAYS</span><h1><span>Master the roadmap.</span><strong>One module at a time.</strong></h1><p>A complete path from first principles to a job-ready portfolio.</p></div><ProgressRing value={completion}/></div><div className="toolbar"><div className="search">⌕<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search courses and topics…"/></div><select><option>All courses</option><option>In progress</option><option>Completed</option></select></div>
  <section className="course-list">{filtered.map(({c,i})=>{
    ...
    const completedTopicCount=topics.filter(topic=>state.completedTopics.includes(topicProgressId(name,topic))).length;
    ...
    return <article className="course" key={name}>
      ...
      {open===i&&<div className="course-content"><div className="topic-grid">{topics.map((t,j)=>{const topicDone=state.completedTopics.includes(topicProgressId(name,t));return <div className={topicDone?"completed":""} key={t}><button className={`topic-check ${topicDone?"topic-done":""}`} onClick={()=>toggleTopic(name,t)} aria-pressed={topicDone} aria-label={`${topicDone?"Reopen":"Mark"} ${t}${topicDone?"":" complete"}`}>{topicDone?"✓":j+1}</button><div><b>{t}</b><small>{j%3===0?"Lesson + guided practice":"Hands-on exercise"} · {35+(j%3)*10} min{topicDone?" · Completed":""}</small><a className="gfg-link" href={gfgUrl(t)} target="_blank" rel="noreferrer" aria-label={`Learn ${t} on GeeksforGeeks`}>Learn this topic on GeeksforGeeks <span aria-hidden="true">↗</span></a></div></div>})}</div><div className="resource-line"><span>↗ More free resources: official documentation, Kaggle Learn, freeCodeCamp</span><button onClick={()=>notify("Prior-knowledge assessment ready")}>Test prior knowledge</button></div></div>}
    </article>
  })}</section>
